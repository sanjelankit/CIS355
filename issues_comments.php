...
// Fetch comments for this issue - SIMPLEST POSSIBLE QUERY
try {
    $stmt = $conn->prepare("SELECT * FROM iss_comments WHERE iss_id = ? ORDER BY posted_date DESC");
    $stmt->execute([$issue_id]);
...
<head>
    ...
    <title>Comments for Issue #<?= htmlspecialchars($issue_id) ?></title>
    <style>
